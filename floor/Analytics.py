      ...
      if IV['areaNumber'] == SKU['areaNumber']:
         ...
         if HM_Ratio:
            ...
            cv2.putText(image, "Category: %s " % IV['areaNumber'],
                     (x2, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), thickness=1)
            cv2.putText(image,
                        "Quantity Selected: %s (percentage)" % str(HM_Ratio * 100),
                        (x2, y1 + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), thickness=1)
         else:
            cv2.circle(image, (CX, CY), int(P * HM_Ratio), (255, 0, 255), -1)
            cv2.putText(image, "Category: %s " % IV['areaNumber'],
                        (x2, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), thickness=1)
