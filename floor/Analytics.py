...
def hetmap(invoice_file = None,SKU_Storage_file=None,Floor_Map_Path=None):
    ...
    try:
		...
		for idx, (IV, SKU) in enumerate(zip(invoice_data['details'], SKU_storage_data['details'])):
			if IV['Category'] == SKU['Category']:
				...
				P = y2 - CY
				if HM_Ratio:
					cv2.circle(image, (CX, CY), int(
						P * HM_Ratio), (255, 0, 255), -1)
					...
					cv2.circle(image, (CX, CY), int(
						P * HM_Ratio), (255, 0, 255), -1)
					cv2.putText(image,
								"Category: %s, Quantity Selected: %s (percentage)" % (
									IV['Category'], str(HM_Ratio * 100)),
								(x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), thickness=3)
			else:
				continue
		...
		print("Please Give Details of your invoice correctly or Items update Failed or Unable to load the Floor map")
